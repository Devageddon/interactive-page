...
if (reversed == null) { reversed = false; }
	...
	this.frame_0 = function() {
		...
		function onMouseUp(evt) {
		  ...
		  console.log("onMouseUp at", evt.stageX, evt.stageY);
		
		  // clear live preview immediately
		  dynLine.graphics.clear();
		  this.stage.update();
		
		  // raw hits will usually be inner Shape children of your MCs
		  const hits = this.stage.getObjectsUnderPoint(evt.stageX, evt.stageY) || [];
		  console.log(" raw hits:", hits);
		
		  // walk up from each hit until we find a named MovieClip ≠ startClip
		  let targetClip = null;
		  for (let h of hits) {
		    let obj = h;
		    while (obj && obj !== this) {
		  ...
		  if (targetClip) {
		    console.log("connecting", startClip.name, "→", targetClip.name);
		    // draw permanent arc
		    ...
		    console.log("no valid target, cancelling");
		  }
		...
		for (let i = 1; i <= NUM_ITEMS; i++) {
		  ...
		  clip.on("mousedown", (evt) => {
		    ...
		    console.log("start drag from", clip.name, startPos);
